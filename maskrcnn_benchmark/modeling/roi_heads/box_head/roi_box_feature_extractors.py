...
@registry.ROI_BOX_FEATURE_EXTRACTORS.register("FPN2MLP_panet_FeatureExtractor")
class FPN2MLP_panet_FeatureExtractor(nn.Module):
    """
    Heads for FPN for classification
    """
    ...
    def forward(self, x, proposals):
        ...
        for i in range(len(x)):
            x[i] = F.relu(self.fc6[i](x[i].view(x[i].size(0), -1)))
...
#actually panet uses it 
@registry.ROI_BOX_FEATURE_EXTRACTORS.register("FPNXconv1fc_panet_FeatureExtractor")
class FPNXconv1fc_panet_FeatureExtractor(nn.Module):
    """
    Heads for FPN for classification
    """
    ...
    def forward(self, x, proposals):
        ...
        x = self.xconvs(x)
        x = F.relu(self.fc6(x.view(batch_size, -1)), inplace=True)
        return x        
