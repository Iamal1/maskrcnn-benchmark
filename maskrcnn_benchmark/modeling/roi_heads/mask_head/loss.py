...
def project_masks_on_boxes(segmentation_masks, proposals, discretization_size, maskiou_on):
    ...
    for segmentation_mask, proposal in zip(segmentation_masks, proposals):
        ...
        if maskiou_on:
            ...
            ##segmentation_mask.instances.polygons:[PolygonInstance(num_groups=1, image_width=600, image_height=800, ]
            for poly_ in segmentation_mask.instances.polygons[0].polygons:
            ...
            '''
            #type 2
            rle_for_fullarea = mask_util.frPyObjects([p.numpy() for p in segmentation_mask_for_maskratio.instances.polygons[0].polygons], y2-y1, x2-x1)
            full_area = torch.tensor(mask_util.area(rle_for_fullarea).sum().astype(float))
            rle_for_box_area = mask_util.frPyObjects([p.numpy() for p in cropped_mask.instances.polygons[0].polygons], proposal[3]-proposal[1], proposal[2]-proposal[0])
